...
export default function ReactionSection({ initialCount, postId }: { postId: string, initialCount: number }) {
    ...
    useEffect(() => {
        if(data){ SetMyVote(data.GetMyReactionForPost.reaction)
        }
   }, [data])
    const [mutateReaction,_] = useMutation(query, { variables: { PostId: postId} });
    const PerformVoteEvent = (e:MouseEvent<HTMLButtonElement, globalThis.MouseEvent>,vote: Reaction)=>{
        e.preventDefault()
        e.stopPropagation()
        if (MyVote !== vote) {
            mutateReaction({ variables: { reaction: vote } });
            SetMyVote(vote);
